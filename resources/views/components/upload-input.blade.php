...
<div>
    <label for="{{ $for }}" {{ $attributes->merge(['class' => 'w-full h-32 cursor-pointer block border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm flex flex-col items-center justify-center']) }}>
    ...
    <input type="file" id="{{ $for }}" name="{{ $for }}" @disabled($disabled) class="hidden">
